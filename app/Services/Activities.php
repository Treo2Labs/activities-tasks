...
class Activities extends \Espo\Core\Services\Base
{
    ...
    protected function getActivitiesSelectParams(Entity $entity, $scope, array $statusList = [], $isHistory)
    {
        ...
        $select = [
            ...
            ['VALUE:' . $scope, '_scope'],
            ($seed->hasAttribute('assignedUserId') ? ['assignedUserId', 'assignedUserId'] : [
                'VALUE:',
                'assignedUserId'
            ]),
    ...
    public function getEventsForTeams($teamIdList, $from, $to, $scopeList = null)
    {
        ...
        $userList = $this->getEntityManager()->getRepository('User')->select(['id', 'name'])->leftJoin([
            [
                'teams',
                'teams'
            ]
    ...
    public function getUpcomingActivities($userId, $params = array(), $entityTypeList = null, $futureDays = null)
    {
        ...
        $unionPartList = [];
        foreach ($entityTypeList as $entityType) {
            if (!$this->getMetadata()->get(['scopes', $entityType, 'activity']) && $entityType !== 'Task') {
                continue;
            }
